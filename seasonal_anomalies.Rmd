...
This notebook will explore annual anomalies in rainy season start and end dates
across Africa and derive relevant statistics describing these anomalies.
...
For double seasons, it looks like there are quite a few pixels where the onset
or cesation date is either close to zero, or 307. I think these correspond to
pixels and seasons where the onset (or cessation??) was not found in the window.

I need to (a) map these pixels, (b) work out how many there are and whether I
can safely remove them from the sample, (c) check if the seasons/pixels with
a missed onset are the same as those with a missed cessation, and (d) decide
whether I need to change my method to reduce these issues.
...
```
After masking, we can calculate key long-term metrics and combine single and
double into a single layer.

I am unsure whether it makes more sense to use the seasonal rainfall average
for finding double season lengths, or the annual rainfall average. The seasonal
average is more sensitive, but could lead to erroneously high values of total
season length if one season has very low rainfall, when compared to the single
season case. For example, a double season pixel in which one season almost
completely failed could still have a long season length in this method. I should
plot the season lengths with both methods and see what looks more reasonable.

If not, I'll need to find a way to mark undetected seasons as failed, and not
simply detect them as usual. On the other hand, very erratic seasons (without a
max/min in the period) would already give NA values in the season length layer,
thereby contributing to higher season length CV.
