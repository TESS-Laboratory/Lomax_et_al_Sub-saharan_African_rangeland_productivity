...
df_model_sample_clean <- df_model_sample %>%
  ...
  mutate(cover = ordered(cover, levels = c("Grasslands", "Shrublands", "Savannas"))) %>%
...
grass_smooths <- gratia::smooth_estimates(mod2_k12_grass, unconditional = TRUE) %>%
  mutate(cover = "Grasslands",
         ci_low= exp(est - se * 1.96),
         ci_high = exp(est + se * 1.96),
         est = exp(est),
         pptMean = pptMean * 1000)
shrub_smooths <- gratia::smooth_estimates(mod2_k12_shrub, unconditional = TRUE) %>%
  mutate(cover = "Shrublands",
         ci_low= exp(est - se * 1.96),
         ci_high = exp(est + se * 1.96),
         est = exp(est),
         pptMean = pptMean * 1000)
savanna_smooths <- gratia::smooth_estimates(mod2_k12_savanna, unconditional = TRUE) %>%
...
ggsave(here(plot_dir, "annual_gam_plot_exp_by_lc_signif_mod2_k12_alpha.jpg"),
       plot = gam_plot,
