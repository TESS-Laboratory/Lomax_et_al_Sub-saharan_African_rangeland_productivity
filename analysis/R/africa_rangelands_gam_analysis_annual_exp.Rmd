...
mod_ac <- mod_residuals %>%
  ...
  summarise(x = mean(x),
            ...
            across(ends_with("resids"),
                   function(res) {acf(res, plot = F, lag.max = 2)$acf[2]})) %>%
  ungroup()
...
# Now try the one with interactions

mod3 <- read_rds(here("data", "processed", "models", "mod3_annual.rds"))

mod3_residuals <- df_model %>%
  mutate(mod3_resids = residuals(mod3, type = "response"))

mod3_ac <- mod3_residuals %>%
  group_by(cell) %>%
  summarise(x = mean(x),
            y = mean(y),
            ac = acf(mod3_resids, plot = F, lag.max = 2)$acf[2]) %>%
  ungroup()
...
plots <- map(var_list, function(var) {
  ...
  # 
  # alpha_list <- pmin(1, df_bins$n / (nrow(df_model) / 1000)) %>%
  #   rep(4)
  # alpha_list <- alpha_list[1:400]
  ...
  plot <- ggplot(smooth_df, aes(x = var0, y = est,
    ...
    scale_colour_manual(values = c("#CCCCCC", "#bae4b3", "#31a354","#006d2c")) +
...
ggsave(here(plot_dir, "annual_gam_plot_exp.jpg"),
       plot = gam_plot,
