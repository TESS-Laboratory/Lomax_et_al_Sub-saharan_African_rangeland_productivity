...
pptMean_pptCV <- base_plot + geom_point(aes(x = pptMean, y = pptCV), size = 0.2) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = "CV (%)")
pptMean_int <- base_plot + geom_point(aes(x = pptMean, y = pptIntensity), size = 0.2) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = expression(Intensity ~ (mm ~ d^-1)))
pptMean_season <- base_plot + geom_point(aes(x = pptMean, y = seasonLength), size = 0.2) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = "Rainy season\nlength (d)")
pptMean_onset_sd <- base_plot + geom_point(aes(x = pptMean, y = seasonOnsetSD), size = 0.2) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = "Season\nonset SD")
pptMean_ugi <- base_plot + geom_point(aes(x = pptMean, y = pptUgi), size = 0.2) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = "UGi")
pptMean_wet95 <- base_plot + geom_point(aes(x = pptMean, y = pptGt95Pc), size = 0.2) +
...
gpp_sand <- base_plot + geom_point(aes(x = sand, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "Soil sand fraction", y = gpp_expr)
gpp_clay <- base_plot + geom_point(aes(x = clay, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "Soil clay fraction", y = gpp_expr)
gpp_T <- base_plot + geom_point(aes(x = tMean, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "Mean air temperature\n(\u00B0C)", y = gpp_expr)
gpp_fire <- base_plot + geom_point(aes(x = fire, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = expression(Fire ~ frequency ~ (yr^-1)), y = gpp_expr)

gpp_pptMean <- base_plot + geom_point(aes(x = pptMean, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = expression(pptMean ~ (mm ~ yr^-1)), y = gpp_expr)
gpp_pptCV <- base_plot + geom_point(aes(x = pptCV, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "CV of annual\nprecipitation (%)", y = gpp_expr)
gpp_int <- base_plot + geom_point(aes(x = pptIntensity, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = expression(Precipitation ~ intensity ~(mm ~ d^-1)), y = gpp_expr)
gpp_dry <- base_plot + geom_point(aes(x = pptDryDays, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "Fraction of dry\ndays per year", y = gpp_expr)
gpp_ugi <- base_plot + geom_point(aes(x = pptUgi, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "UGi", y = gpp_expr)
gpp_wet95 <- base_plot + geom_point(aes(x = pptGt95Pc, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = expression(F["95w"]), y = gpp_expr)
gpp_season <- base_plot + geom_point(aes(x = seasonLength, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "Wet season\nlength (days)", y = gpp_expr)
gpp_onset_sd <- base_plot + geom_point(aes(x = seasonOnsetSD, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "SD of wet\nseason onset (days)", y = gpp_expr)
gpp_mean_day_sd <- base_plot + geom_point(aes(x = pptMeanDaySD, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = "SD of mean rainfall day (days)", y = gpp_expr)
gpp_soilN <- base_plot + geom_point(aes(x = soilN, y = gpp), size = 0.2, alpha = 0.75) +
  labs(x = expression(Soil ~ nitrogen ~ (g ~ kg^-1)), gpp_expr)
gpp_soilP <- base_plot + geom_point(aes(x = soilP, y = gpp), size = 0.2, alpha = 0.75) +
...
africa_basemap <- tm_shape(africa) + tm_borders() +
  ...
  tm_graticules(x = c(-15, 0, 15, 30, 45), y = c(-30, -15, 0, 15, 30),
                col = "grey60", labels.size = 0.4, lwd = 0.75)
