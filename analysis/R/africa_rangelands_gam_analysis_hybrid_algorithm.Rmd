...
source(here("analysis", "R", "feature_selection", "HybridFunctions_Parallelized_mod.R"))
...
gpp_mean <- rast(here("data", "raw", "raster", "main_variables",
                      "gppMean.tif")) * 0.01

# CHIRPS precipitation variables
map <- rast(here("data", "raw", "raster", "main_variables",
                 "pptMean.tif"))
cv <- rast(here("data", "raw", "raster", "main_variables",
                "pptCV.tif"))
intensity <- rast(here("data", "raw", "raster", "main_variables",
                "pptIntensity.tif"))
dry <- rast(here("data", "raw", "raster", "main_variables",
                "fracDryDays.tif"))
ugi <- rast(here("data", "raw", "raster", "main_variables",
                "ugi.tif"))
pci <- rast(here("data", "raw", "raster", "main_variables",
                "pci.tif"))
pc_95 <- rast(here("data", "raw", "raster", "main_variables",
                "pptF95.tif"))
wet_95 <- rast(here("data", "raw", "raster", "main_variables",
                "pptF95W.tif"))
gt10 <- rast(here("data", "raw", "raster", "main_variables",
                "pptExceed_10.tif"))
gt20 <- rast(here("data", "raw", "raster", "main_variables",
                "pptExceed_20.tif"))
gt30 <- rast(here("data", "raw", "raster", "main_variables",
                "pptExceed_30.tif"))
mean_day_sd <- rast(here("data", "raw", "raster", "main_variables",
                         "pptMeanDaySD.tif"))

# CHIRPS season length and onset data 
season_length <- rast(here("data", "processed", "raster",
                           "season_length_mean.tif"))
season_length_sd <- rast(here("data", "processed", "raster",
                              "season_length_sd.tif"))
season_onset_sd <- rast(here("data", "processed", "raster",
                             "season_onset_sd.tif"))

# ERA5-Land mean air temperature
tMean <- rast(here("data", "raw", "raster", "main_variables", "tMean.tif"))

# MODIS fire frequency 2001-2019
fire <- rast(here("data", "raw", "raster", "main_variables", "modisBurnFreq.tif")) %>%
...
esa_other <- rast(here("data", "raw", "raster", "study_area_masks",
                  "esaLcFrac.tif"))
esa_woody <- rast(here("data", "raw", "raster", "study_area_masks",
                  "esaWoodyFrac.tif"))
...
# Invalid season identification mask (remove values with NA for season onset sd)
season_na_mask <- !is.na(season_onset_sd)

# Combine masks
study_area_mask <- igbp_mode & ai_mask & esa_mask & zero_mask & season_na_mask
...
write_csv(df, here("data", "processed", "csv", "full_df_multi_annual.csv"))
...
df <- read_csv(here("data", "processed", "csv", "full_df_multi_annual.csv"))

cor_matrix = df %>%
  dplyr::select(gpp,map,cv,intensity,dry_days,ugi,pci,pc95,wet95,gt10,gt30,
                mean_day_sd,season,onset_sd,sand,clay,soilN,soilP,tMean,fire) %>%
  cor(method = "spearman", use="pairwise.complete.obs")

var_names <- c("GPP", "MAP", "CV", "SDII", "dry", "UGi", "PCI", ":F[95]",
               ":F['95w']", ":D['10mm']", ":D['30mm']", "meanDaySD", "season",
               ":sigma[onset]","sand", "clay", "soilN", "soilP", "tMean", "fire")
colnames(cor_matrix) <- var_names
rownames(cor_matrix) <- var_names

# Save correlation plot figure to directory

png(here(plot_dir, "cor_plot_2.png"),
...
Let's try using the Kovacs 2022 algorithm to automatically select features
while avoiding concurvity

```{r feature_selection}

# Set up simplified dataset for feature selection
df <- read_csv(here("data", "processed", "csv", "full_df_multi_annual.csv"))

set.seed(123)
df_sample <- sample_frac(df, 0.05)

target_var <- df_sample %>%
  select(gpp) %>%
  as_vector()

feature_df <- df_sample %>%
  select(map,cv,intensity,dry_days,ugi,pci,pc95,wet95,gt10,gt30,
         mean_day_sd,season,onset_sd,sand,clay,soilN,soilP,tMean,fire)

# Run feature selection algorithm using modified Kovacs 2022 algorithm
Sys.time()
tic()
best_features <- Hibrid(
  genszam = 19,                  # Number of possible features
  pop_meret = 32,                # Size of population of models per iteration
  maxlepes = 100,                # Max number of iterations
  mutacio = 90,                  # -----
  HMCR = 5,                      # Search parameters (using recommended values
  vegmutacio = 10,               # from Kovacs 2022)
  vegHMCR = 35,                  # -----
  konvergKrit = 10,              # Early stopping criterion (number of iters)
  X = feature_df,                # Data frame containing features
  Y = target_var,                # Vector containing target variable
  csalad = Gamma(link = "log"),  # Model family
  faktorok = NULL,               # Vector of factor variables (NA in this case)
  konkurv_threshold = 0.5,       # Maximum allowed model-scale concurvity
  konkurv_strict = 3,            # Worst (1), observed (2) or estimated (3) concurvity
  magok = 32                     # Number of cores for parallelisation
)
Sys.time()
toc()
...
saveRDS(best_features, here("data", "processed", "rds", "best_features.rds"))

best_feature_logical <- str_split(best_features[[1]], ", ") %>%
  unlist()

best_feature_indices <- which(best_feature_logical == "1")

best_feature_names <- colnames(feature_df)[best_feature_indices]

cl <- makeCluster(16)
test_gam <- bam(gpp ~ s(map) + s(intensity) + s(dry_days) + s(wet95) + s(gt30) + 
                  s(mean_day_sd) + s(season) + s(onset_sd) + s(clay) + s(soilN) +
                  s(soilP) + s(tMean) + s(fire),
                data = df_sample,
...
df_model <- df %>%
  dplyr::select(x, y, gpp, map, cv, ugi, season, onset_sd, mean_day_sd,
                intensity, wet95, soilN, sand, tMean, fire) %>%
...
write_csv(df_model,
          here("data", "processed", "csv", "df_model.csv"))
...
df_model <- read_csv(here("data", "processed", "csv", "df_model.csv"))

cl <- makeCluster(nc)

tic()
mod1 <- bam(gpp ~ s(map, k = 16, bs = "ts") + s(cv, k = 12, bs = "ts") +
              s(intensity, k = 12, bs = "ts") + s(wet95, k = 12, bs = "ts") +
              s(ugi, k = 12, bs = "ts") + s(mean_day_sd, k = 12, bs = "ts") +
              s(season, k = 12, bs = "ts") + s(onset_sd, k = 12, bs = "ts") +
              s(soilN, k = 12, bs = "ts") + s(sand, k = 12, bs = "ts") +
              s(tMean, k = 12, bs = "ts") +
              s(fire, k = 12, bs = "ts"),
            data = df_model,
            method = "REML",
...
tic()
mod2_k8 <- bam(gpp ~ s(map, k = 16, bs = "ts") + s(cv, k = 12, bs = "ts") +
              s(intensity, k = 12, bs = "ts") + s(wet95, k = 12, bs = "ts") +
              s(ugi, k = 12, bs = "ts") + s(mean_day_sd, k = 12, bs = "ts") +
              s(season, k = 12, bs = "ts") + s(onset_sd, k = 12, bs = "ts") +
              s(soilN, k = 12, bs = "ts") + s(sand, k = 12, bs = "ts") +
...
tic()
mod2_k12 <- bam(gpp ~ s(map, k = 16, bs = "ts") + s(cv, k = 12, bs = "ts") +
              s(intensity, k = 12, bs = "ts") + s(wet95, k = 12, bs = "ts") +
              s(ugi, k = 12, bs = "ts") + s(mean_day_sd, k = 12, bs = "ts") +
              s(season, k = 12, bs = "ts") + s(onset_sd, k = 12, bs = "ts") +
              s(soilN, k = 12, bs = "ts") + s(sand, k = 12, bs = "ts") +
...
tic()
mod2_k16 <- bam(gpp ~ s(map, k = 16, bs = "ts") + s(cv, k = 12, bs = "ts") +
              s(intensity, k = 12, bs = "ts") + s(wet95, k = 12, bs = "ts") +
              s(ugi, k = 12, bs = "ts") + s(mean_day_sd, k = 12, bs = "ts") +
              s(season, k = 12, bs = "ts") + s(onset_sd, k = 12, bs = "ts") +
              s(soilN, k = 12, bs = "ts") + s(sand, k = 12, bs = "ts") +
...
df_model_bins <- df_model %>%
  mutate(map_bin = cut_width(map, width = 0.4, boundary = 0)) %>%
  group_by(map_bin) %>%
  nest() %>%
  ungroup()
...
fit_gam <- function(data, spatial = FALSE, spatial_k = 12) {
  ...
  if (spatial) {
    model <- bam(gpp ~ s(map, k = 16, bs = "ts") + s(cv, k = 12, bs = "ts") +
                  s(intensity, k = 12, bs = "ts") + s(wet95, k = 12, bs = "ts") +
                  s(ugi, k = 12, bs = "ts") + s(mean_day_sd, k = 12, bs = "ts") +
                  s(season, k = 12, bs = "ts") + s(onset_sd, k = 12, bs = "ts") +
                  s(soilN, k = 12, bs = "ts") + s(sand, k = 12, bs = "ts") +
...
models_binned <- df_model_bins %>%
  mutate(nsp_models = future_map(data, fit_gam, spatial = FALSE),
         sp_models_k8 = future_map(data, fit_gam, spatial = TRUE, spatial_k = 8),
         sp_model_k12 = future_map(data, fit_gam, spatial = TRUE, spatial_k = 12),
         sp_model_k16 = future_map(data, fit_gam, spatial = TRUE, spatial_k = 16))
...
models_binned_cut <- models_binned[1:4,]
