...
ModellEpit=function(egyed, X, Y, csalad, faktorok, konkurv_strict, konkurv_threshold, magok){
  ...
  tryCatch({
    ...
    for(i in 1:length(konkurv)){
      if(konkurv[i]>konkurv_threshold) {vifre<-FALSE
      break}
    }
  }, warning = function(w) {
...
Hibrid=function(genszam,pop_meret,maxlepes,mutacio, HMCR, vegmutacio, vegHMCR, konvergKrit, X, Y, csalad, faktorok, konkurv_threshold, konkurv_strict, magok){
  ...
  for(index in 1:maxlepes){
    ...
    ujpop[startIndex:pop_meret,] <- foreach (j = startIndex:pop_meret, .combine='rbind', .export="ModellEpit") %dopar% {
      ...
      if(sum(egyed.akt)==0){
        ...
        mod.out<-ModellEpit(egyed.akt,X,Y, csalad, faktorok, konkurv_strict, konkurv_threshold, magok)
        c(toString(egyed.akt),mod.out[1],mod.out[2],mod.out[3])
      }
    }
    stopCluster(cl)
