...
mean_vars <- c("gpp", "pptTotal", "pptIntensity", "pptGt95Pc",
                   "pptGt10", "pptGt30", "pptUgi", "tMean",
...
df_multi_annual <- df %>%
  ...
  summarise(pptCV = sd(pptTotal) / mean(pptTotal),
            ...
            across(all_of(vars_for_mean), mean)) %>%
  ungroup() %>%
...
df_annual <- df %>%
  ...
  mutate(pptMean = mean(pptTotal),
         ...
         pptAnomalyFrac = pptTotal / pptMean,
...
```{r gam_prep, eval = FALSE}

# Select model variables and scale some to more consistent units (0-1)
df_model <- df_annual %>%
  dplyr::select(cell, x, y, year, gpp, pptTotal, pptMean, pptAnomaly, pptIntensity,
                pptGt95Pc, pptMeanDayAnomaly, pptUgi, seasonLength,
                seasonOnsetAnomaly, soilN, sand, tMean, fire) %>%
  mutate(gpp = gpp * 0.001,
         sand = sand * 0.01,
         pptMean = pptMean * 0.001,
         pptTotal = pptTotal * 0.001)
...
# set.seed(123)
# test_df <- sample_frac(df_model, 0.1)

tic()
mod1 <- bam(gpp ~ s(pptTotal, k = 16, bs = "ts") +
              s(pptMeanDayAnomaly, k = 12, bs = "ts") +
              # ti(pptTotal, pptMeanDayAnomaly, k = c(12,12), bs = "ts") +
              s(pptIntensity, k = 12, bs = "ts") +
              s(pptGt95Pc, k = 12, bs = "ts") +
              s(pptUgi, k = 12, bs = "ts") +
              s(seasonLength, k = 12, bs = "ts") +
              s(seasonOnsetAnomaly, k = 12) +
              s(tMean, k = 12, bs = "ts") +
              s(fire, k = 12, bs = "ts") +
              s(sand, k = 12, bs = "ts") +
              s(soilN, k = 12, bs = "ts") +
...
write_rds(mod1, here("data", "processed", "models", "mod1_annual_abs.rds"))


```


``` {r raw_precipitation_model_viz}

mod1 <- read_rds(here("data", "processed", "models", "mod1_annual.rds"))
...
Second model splitting precipitation into mean annual precipitation and
annual precipitation anomaly:

```{r precipitation_anomaly_model}
...
mod2 <- bam(gpp ~ s(pptMean, k = 16, bs = "ts") +
              ...
              s(pptMeanDayAnomaly, k = 12, bs = "ts") +
              # ti(pptAnomaly, pptMeanDayAnomaly, k = 12, bs = "ts") +
              s(pptIntensity, k = 12, bs = "ts") +
              s(pptGt95Pc, k = 12, bs = "ts") +
              s(pptUgi, k = 12, bs = "ts") +
              s(seasonLength, k = 12, bs = "ts") +
...
write_rds(mod2, here("data", "processed", "models", "mod2_annual_abs.rds"))

```


``` {precipitation_anomaly_model_viz}

mod2 <- read_rds(here("data", "processed", "models", "mod2_annual.rds"))
