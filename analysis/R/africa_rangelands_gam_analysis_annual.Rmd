...
models_binned <- df_model_bins %>%
  mutate(mod1 = map(data, fit_gam, spatial = FALSE),
         mod2_k8 = map(data, fit_gam, spatial = TRUE, spatial_k = 8),
         mod2_k12 = map(data, fit_gam, spatial = TRUE, spatial_k = 12),
         mod2_k16 = map(data, fit_gam, spatial = TRUE, spatial_k = 16))
toc()
stopCluster(cl)

write_rds(models_binned, here("results", "rds", "binned_models_annual.rds"))
