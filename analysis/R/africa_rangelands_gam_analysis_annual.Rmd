...
lc_mask <- rast(here("data", "processed", "raster", "var_raster_stack.tif")) %>%
  subset("lc")

shrub_mask <- lc_mask == 7
savanna_mask <- lc_mask == 9
grass_mask <- lc_mask == 10

lc_list <- list(shrub_mask, savanna_mask, grass_mask)

# Load spatial smooths from different models
spatial_smooth_shrub <- gratia::smooth_estimates(
  mod2_k16_shrub,
  n = 300,
  smooth = "te(x,y)",
  dist = 0.01) %>%
  ...
  mutate(est = exp(est)) %>%
  select(x, y, est)
spatial_smooth_savanna <- gratia::smooth_estimates(
  mod2_k16_savanna,
  n = 300,
  smooth = "te(x,y)",
  dist = 0.01) %>%
  ...
  mutate(est = exp(est)) %>%
  select(x, y, est)
spatial_smooth_grass <- gratia::smooth_estimates(
  mod2_k16_grass,
  n = 300,
  smooth = "te(x,y)",
  dist = 0.01) %>%
...
# Combine into single SpatRaster layer
spatial_smooths_list <- list(spatial_smooth_shrub, spatial_smooth_savanna, spatial_smooth_grass)

spatial_rast <- purrr::map2(spatial_smooths_list, lc_list, function(df, lc_mask) {
  r <- rast(df, crs = "epsg:4326") %>%
    resample(lc_mask) %>%
    mask(lc_mask, maskvalues = c(0, NA))
  r
}) %>% purrr::reduce(merge)
...
rast_plot <- tm_shape(spatial_rast) +
  tm_raster(
    ...
    col.legend = tm_legend(
      title = "Partial effect of spatial term",
...
var_labels_df <- tibble(
  var = signif_vars,
  var_label = ordered(signif_var_labels, levels = signif_var_labels)
)
...
# Save to disk
ggsave(here("results", "figures", "plot_binned_by_lc_signif_vars.jpg"),
       plots_binned,
