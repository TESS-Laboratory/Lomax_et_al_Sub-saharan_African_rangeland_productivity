...
gpp_mean <- rast(here("data", "raw_data", "raster", "PML_V2", 
                      "pmlv2_gpp_mean.tif")) * 0.01

# CHIRPS precipitation variables
map <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_MAP.tif"))
cv <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_annual_CV.tif"))
intensity <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_intensity.tif"))
dry <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_dry_days.tif"))
ugi <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_UGi.tif"))
pci <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_PCI.tif"))
pc_95 <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_95pc.tif"))
wet_95 <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_wet_95pc.tif"))
gt10 <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_exceed_10.tif"))
gt20 <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_exceed_20.tif"))
gt30 <- rast(here("data", "raw_data", "raster", "CHIRPS", "chirps_exceed_30.tif"))

# CHIRPS season length and onset data 
season_length <- rast(here("data", "processed_data", "raster", "seasonality",
                           "season_length.tif"))
season_length_sd <- rast(here("data", "processed_data", "raster", "seasonality",
                              "season_length_sd.tif"))
season_onset_sd <- rast(here("data", "processed_data", "raster", "seasonality",
                             "season_onset_sd.tif"))

# ERA5-Land mean air temperature
meanT <- rast(here("data", "raw_data", "raster", "ERA5-Land", "era5_T.tif"))

meanT_resample <- disagg(meanT, 2, method = "bilinear")

# MODIS mean daytime land surface temperature
lst <- rast(here("data", "raw_data", "raster", "MODIS", "modis_lst.tif"))

# MODIS fire frequency 2001-2019
fire <- rast(here("data", "raw_data", "raster", "MODIS", "modis_burn_freq.tif")) %>%
  classify(matrix(c(NA, 0), nrow = 1))

# iSDAsoil data (top 20cm)
soil <- rast(here("data", "raw_data", "raster", "iSDAsoil", "isda_0_20_cm.tif")) %>%
  crop(map) # Fixing error in extent from export
names(soil) <- c("sand", "clay", "N", "P")

# Mask layers
# Aridity index
ai <- rast(here("data", "raw_data", "raster", "WorldClim", "aridity.tif"))

# MODIS land cover classification IGBP
igbp <- rast(here("data", "raw_data", "raster", "MODIS", "IGBP_land_cover.tif"))

# ESA WorldCover 10m land cover masks (2020)
# Additional higher-resolution layers excluding urban, cropland and forested land
esa_other <- rast(here("data", "raw_data", "raster", "ESA", "ESA_lc_frac.tif"))
esa_tree <- rast(here("data", "raw_data", "raster", "ESA", "ESA_tree_frac.tif"))

# Zero mask (remove pixels where more than 25% of sub-pixels are zero for all
# years in the dataset)
zero_mask <- rast(here("data", "raw_data", "raster", "PML_V2", "pmlv2_zero_mask.tif"))

# CHIRPS seasonality ratio (for figures)
# sr > 1 indicates multiple rainy seasons per year
sr <- rast(here("data", "raw_data", "raster", "chirps", "seasonality_ratio.tif"))
...
# writeRaster(igbp_mode, here("data", "processed_data", "raster", "modis",
#                             "igbp_mode.tif"),
#             overwrite = TRUE)

igbp_mode <- rast(here("data", "processed_data", "raster", "modis", "igbp_mode.tif"))
...
png(paste0(plot_dir, "/cor_plot.png"),
...
# write_csv(df_model,
#           here("data", "processed_data", "csv", "df_model.csv"))
# df_model <- read_csv(here("data", "processed_data", "csv", "df_model.csv"))
