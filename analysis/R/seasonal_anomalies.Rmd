---
title: Disentangling the environmental drivers of gross primary productivity in African rangelands - Seasonal anomaly calculation
author: Guy Lomax
date: 2023-05-31
...
onset_anomaly_s <- season_dates_anomaly_s[[grepl("onset", names(season_dates_anomaly_s))]]

a <- sum(values(onset_anomaly_s) < -60 | values(onset_anomaly_s) > 60, na.rm = TRUE)
b <- sum(!is.na(values(onset_anomaly_s)))
print("Single season onset")
a/b

# Double season onset dates
onset_anomaly_d <- season_dates_anomaly_d[[grepl("onset", names(season_dates_anomaly_d))]]


c <- sum(values(onset_anomaly_d) < -45 | values(onset_anomaly_d) > 45, na.rm = TRUE)
d <- sum(!is.na(values(onset_anomaly_d)))
print("Double season onset")
c/d

# Single season cessation dates
cessation_anomaly_s <- season_dates_anomaly_s[[grepl("cessation", names(season_dates_anomaly_s))]]

# summary(cessation_anomaly)
e <- sum(values(cessation_anomaly_s) < -60 | values(cessation_anomaly_s) > 60, na.rm = TRUE)
f <- sum(!is.na(values(cessation_anomaly_s)))
print("Single season cessation")
e/f

# Double season cessation dates
cessation_anomaly_d <- season_dates_anomaly_d[[grepl("cessation", names(season_dates_anomaly_d))]]

# summary(cessation_anomaly)
g <- sum(values(cessation_anomaly_d) < -45 | values(cessation_anomaly_d) > 45, na.rm = TRUE)
...
onset_error_pixels_d <- sum(onset_anomaly_d < -45 | onset_anomaly_d > 45)
cessation_error_pixels_d <- sum(cessation_anomaly_d < -45 | cessation_anomaly_d > 45)
...
anomaly_mask_d <- (onset_anomaly_d >= -45 &
                          onset_anomaly_d <= 45) & 
                       (cessation_anomaly_d >= -45 &
                          cessation_anomaly_d <= 45)
...
mean_length <- merge(mean_length_s, mean_length_d)
...
writeRaster(season_length_sd, "processed/raster/season_length_sd.tif",
            overwrite = TRUE)
writeRaster(season_onset_sd, "processed/raster/season_onset_sd.tif",
            overwrite = TRUE)
