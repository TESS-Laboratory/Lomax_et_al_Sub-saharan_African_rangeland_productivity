...
hYear_start <- rast(here("data", "raw_data", "raster", "season_variables",
                           "hYearStartMean_Export.tif")) %>%
  crop(season_length)
...
annual_season_dates_s <- rast(here("data", "raw_data", "raster", "season_variables",
                            "annualSeasonDatesSingle.tif"))

date_names_s <- c("onset_", "cessation_")
year_names_s <- as.character(rep(2001:2019, each = 2))

layer_names <- paste0(date_names_s, year_names_s)

names(season_dates_s) <- layer_names_s

# Seasonal rainfall dates and anomalies - double seasons
annual_season_dates_d <- rast(here("data", "raw_data", "raster", "season_variables",
                            "annualSeasonDatesDouble.tif"))
...
seasonality_ratio <- rast(here("data", "raw_data", "raster", "season_variables",
...
# Express as dates in hydrological rather than calendar year
season_dates_s_hYear <- season_dates_s - hYear_start
season_dates_d_hYear <- season_dates_d - hYear_start
...
writeRaster(season_length_sd, "processed_data/raster/season_length_sd.tif",
            overwrite = TRUE)
writeRaster(season_onset_sd, "processed_data/raster/season_onset_sd.tif",
            overwrite = TRUE)
