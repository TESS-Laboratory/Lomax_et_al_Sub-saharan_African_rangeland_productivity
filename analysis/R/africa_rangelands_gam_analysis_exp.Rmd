...
ggsave(here(plot_dir, "main_gam_plot_exp_alpha.jpg"),
       plot = gam_plot,
...
write_rds(mod1_binned, here("data", "processed", "models", "mod1_binned.rds"))

mod2_k8_binned <- df_model_bins %>%
  mutate(mod2_k8 = map(data, fit_gam, spatial = TRUE, spatial_k = 8)) %>%
  select(-data)
write_rds(mod2_k8_binned, here("data", "processed", "models", "mod2_k8_binned.rds"))

mod2_k12_binned <- df_model_bins %>%
  mutate(mod2_k12 = map(data, fit_gam, spatial = TRUE, spatial_k = 12)) %>%
  select(-data)
write_rds(mod2_k12_binned, here("data", "processed", "models", "mod2_k12_binned.rds"))

mod2_k16_binned <- df_model_bins %>%
  mutate(mod2_k16 = map(data, fit_gam, spatial = TRUE, spatial_k = 16)) %>%
  select(-data)
write_rds(mod2_k16_binned, here("data", "processed", "models", "mod2_k16_binned.rds"))

toc()
stopCluster(cl)

mod1_binned <- read_rds(here("data", "processed", "models", "mod1_binned.rds"))
mod2_k8_binned <- read_rds(here("data", "processed", "models", "mod2_k8_binned.rds"))
mod2_k12_binned <- read_rds(here("data", "processed", "models", "mod2_k12_binned.rds"))
mod2_k16_binned <- read_rds(here("data", "processed", "models", "mod2_k16_binned.rds"))

models_binned_all <- mod1_binned %>%
  left_join(mod2_k8_binned) %>%
  left_join(mod2_k12_binned) %>%
  left_join(mod2_k16_binned) %>%
  slice(1:3)

write_rds(models_binned_all, here("data", "processed", "models", "models_binned_all.rds"))
